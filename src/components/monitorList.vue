      ...
      <div class="fullWrapper">
        <div class="addDialog">
          <div class="closeBtn" @click="closeDialog"></div>
					...
					<el-carousel trigger="click" height="4.2rem" :autoplay="false" indicator-position="outside">
						<el-carousel-item v-for="(v,vi) in totalList" :key="vi">
							<div class="gridWrapper">
								<div class="environmentControl" style="padding-left: .13rem;" v-for="(el,i) in v" :key="i">
									...
									<div class="botttom" style="padding-bottom:.15rem;">
										<div class="environmentBk">
											<div class="circle">
												<span class="level" v-if="el.meterLevelObj.level == '优'">{{ el.meterLevelObj.level }}</span>
...
export default {
      ...
      immediate: true
		...
		'dialogTableVisible': function(n){
			...
			// 关闭options中的所有监控
				this.totalList.forEach((v,i) => {
					v.forEach((el,index) => {
            this.wsLeftArr[i][index].meterList && this.wsLeftArr[i][index].meterList.onclose(true);
            this.wsLeftArr[i][index].meterLevelObj && this.wsLeftArr[i][index].meterLevelObj.onclose(true);
					})
				})
			}
		}
    ...
    setMeterLevelObj(deviceId,index,secondIndex) {
      ...
      this.wsLeftArr[index][secondIndex].meterLevelObj.onclose = function(flag) {
        // 关闭 websocket
